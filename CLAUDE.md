...
## Sitios Instalados (Produccion)

### Mall Barrio Independencia (MBI)

**Cliente**: Mall Barrio Independencia
**Instalado**: 2026-01-26
**Metodo**: rsync desde servidor local

#### Acceso SSH

```bash
# Conexion directa
sshpass -p 'Raul Labbe 14050h' ssh -o StrictHostKeyChecking=no root@64.176.20.15

# O usar variables
VPS_IP="64.176.20.15"
VPS_USER="root"
VPS_PASS="Raul Labbe 14050h"
```
...
| Servicio | URL |
|----------|-----|
| **Frontend** | https://mbi.mediaflow.cl |
| **API** | https://mbi.mediaflow.cl/api/v1/ |
| **VPS IP** | 64.176.20.15 |
...
/var/www/mediaflow/
├── backend/           # API FastAPI
│   ├── .env           # Configuracion (NO tocar API keys)
│   └── venv/          # Entorno virtual Python
├── frontend/
│   ├── dist/          # Build de produccion
│   └── public/tenants/mallbarrio/  # Assets del cliente
└── storage/
    ├── audio/         # Audios generados
    ├── music/         # Tracks de musica
...
#### Backstory de la Instalacion

1. **Problema inicial**: Backend en puerto 8000 (conflicto con Azuracast)
   - **Solucion**: Cambiado a puerto 3001

2. **Problema ffprobe**: Error "ffprobe not found"
   - **Causa**: PATH del servicio systemd no incluia /usr/bin
   - **Solucion**: Agregar `/usr/bin:/usr/local/bin` al PATH en mediaflow.service

3. **Problema modelo Claude**: Error 404 modelo no existe
   - **Causa**: `claude-3-5-sonnet-20241022` deprecado
   - **Solucion**: Cambiado a `claude-sonnet-4-20250514`

4. **Musica**: No habia tracks de musica
   - **Solucion**: rsync de archivos + INSERT en PostgreSQL

5. **Logo**: Actualizado logo del cliente
   - Archivo: `/var/www/mediaflow/frontend/public/tenants/mallbarrio/logo.png`
   - Landing page actualizado (hardcoded)

#### Comandos Utiles para MBI

```bash
# Conectar al VPS
sshpass -p 'Raul Labbe 14050h' ssh root@64.176.20.15

# Ver logs en tiempo real
sshpass -p 'Raul Labbe 14050h' ssh root@64.176.20.15 "journalctl -u mediaflow -f"

# Verificar API
sshpass -p 'Raul Labbe 14050h' ssh root@64.176.20.15 "curl -s http://localhost:3001/api/v1/config/tenant"

# rsync actualizacion de codigo
sshpass -p 'Raul Labbe 14050h' rsync -avz --exclude 'venv' --exclude '__pycache__' --exclude '.env' \
    /var/www/mediaflow-v2/backend/ root@64.176.20.15:/var/www/mediaflow/backend/
```
...
sshpass -p 'Raul Labbe 14050h' ssh root@64.176.20.15 "sudo -u postgres psql mediaflow"
