"""
Automatic Playroom API Endpoints
Experimental clone of Automatic Mode for UI testing - 100% independent
"""
import os
import uuid
import shutil
...
@router.get(
    ...
    summary="Get Playroom Config",
    description="Get default configuration for playroom (same as automatic mode)",
)
async def get_playroom_config(
    db: AsyncSession = Depends(get_db),
):
    """Get configuration for playroom"""
    try:
        logger.info("⚙️ Fetching playroom configuration")
        ...
        logger.error(f"❌ Failed to fetch playroom config: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch playroom config: {str(e)}",
        )
...
@router.post(
    ...
    summary="Generate Playroom Jingle",
    description="Generate a jingle in playroom mode (same as automatic)",
)
async def generate_playroom_jingle(
    ...
    """
    try:
        logger.info(f"🎙️ [PLAYROOM] Starting generation: voice={request.voice_id}, duration={request.target_duration}s")
        ...
        if request.improve_text:
            logger.info("🤖 [PLAYROOM] Improving text with Claude AI...")
            ...
            try:
                ...
                logger.info(f"✅ [PLAYROOM] Text improved: {word_count} words (target: {min_words}-{max_words})")

            except Exception as ai_error:
                logger.warning(f"⚠️ [PLAYROOM] AI improvement failed, using original text: {ai_error}")
                improved_text = original_text

        # 3. Generate TTS with ElevenLabs
        logger.info("🔊 [PLAYROOM] Generating TTS audio...")
        ...
        # 4. Generate unique filename (with "playroom" prefix for easy identification)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        unique_id = str(uuid.uuid4())[:8]
        filename = f"playroom_{timestamp}_{request.voice_id}_{unique_id}.mp3"
        ...
        if has_music:
            logger.info(f"🎵 [PLAYROOM] Mixing with music: {request.music_file}")
            ...
            if not jingle_result['success']:
                ...
                logger.warning(f"⚠️ [PLAYROOM] Jingle creation failed: {jingle_result.get('error')}, saving TTS only")
        ...
        # Save to database (with playroom prefix in display name for easy filtering)
        audio_message = AudioMessage(
            filename=filename,
            display_name=f"[PLAYROOM] {display_name}",
        ...
        logger.info(f"✅ [PLAYROOM] Jingle generated: {filename} ({final_duration:.2f}s) - ID={audio_message.id}")
        ...
        logger.error(f"❌ [PLAYROOM] Generation failed: {str(e)}", exc_info=True)
        return AutomaticGenerateResponse(
            success=False,
