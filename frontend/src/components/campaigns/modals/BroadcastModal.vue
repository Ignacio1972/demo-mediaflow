      ...
      <div class="modal-action">
        ...
        >
          ...
          <template v-else>
            ...
            Enviar
            <span v-if="selectedBranches.length > 0" class="badge badge-sm bg-white/20 border-0 text-white">
...
async function send() {
  if (selectedBranches.value.length === 0 || !props.audioId) return
  ...
  try {
    ...
    const result = await libraryApi.sendToRadio(props.audioId, true)
