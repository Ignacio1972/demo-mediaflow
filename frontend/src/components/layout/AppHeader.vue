<template>
  <header class="fixed top-4 left-4 right-4 h-16 bg-base-100 rounded-xl shadow-theme-sm z-50 flex items-center">
    <!-- Mobile: Burger -->
    <button
      class="lg:hidden btn btn-ghost btn-sm btn-square ml-2"
      @click="openSidebar"
    ...
    <div class="hidden lg:block w-px h-8 bg-base-content/10 shrink-0" />

    <!-- Desktop: Page title -->
    <h1 class="hidden lg:block text-lg font-medium text-base-content ml-4">
      {{ pageTitle }}
    </h1>
...
onMounted(() => {
  const savedTheme = localStorage.getItem('theme') || 'tailadmin'
